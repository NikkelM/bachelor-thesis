...
%    \boldNumberSetsfalse % Use this value if you want variables for number sets (like N or R) to appear in blackboard bold rather than bold.
    ...
    \myTitle{Retaining maintainability throughout the scaling of software projects using automated tools}{Deutscher Titel}
    ...
    \myKeywords{bachelor thesis | reinforcement learning | python | dynamic pricing}
...
\chapter{Knowing your Users(?)}
\input{to_be_ordered/users}
...
\chapter{Prvate repository (less tools)(?)}

\chapter{Documentation}
\input{to_be_ordered/documentation}

\chapter{Tests}
\input{to_be_ordered/tests}

\part{Automation Tools}

\chapter{Pre-commit}
\input{to_be_ordered/pre-commit}

\chapter{Github Actions}

\chapter{Sphinx/Readthedocs}

\part{Packaging the Project}

\chapter{Why package the project?}

\chapter{Using Pip}
\input{to_be_ordered/pip}
