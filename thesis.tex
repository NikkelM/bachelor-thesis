...
%    \boldNumberSetsfalse % Use this value if you want variables for number sets (like N or R) to appear in blackboard bold rather than bold.
    ...
    \myTitle{Monitoring of Agents for Dynamic Pricing in different Recommerce Markets}
	{Monitoring von Agenten zur dynamischen Bepreisung in unterschiedlichen Recommerce-Märkten}
    ...
    \myKeywords{bachelor thesis | reinforcement learning | rl | python | dynamic pricing | recommerce | monitoring | simulation}
...
\input{chapters/03_simulating_marketplace}

\chapter[Approaches to Monitoring Agents]{Approaches to\\Monitoring Agents}
\input{chapters/04_approaches}

\chapter{The \emph{recommerce} Workflow}
\input{chapters/05_workflow}

\chapter{Monitoring an Experiment}
\input{chapters/06_monitoring_experiment.tex}
