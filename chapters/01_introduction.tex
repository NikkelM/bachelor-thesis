...
In this chapter, we will first introduce the general concept of a \emph{recommerce} market (\nameref{sec:CircularEconomy}) as well as \emph{Reinforcement-Learning}.\todo{Do some motivation for the topic? e.g. using the statista statistic} In \fullref{ch:RelatedWork} we will explore the difficulties faced in dynamic pricing and current and novel approaches to monitoring and evaluating Reinforcement-Learning agents in different circumstances. This will be followed by an overview of the specific features of the \emph{recommerce} market which we implemented in \fullref{ch:SimulatingMarketplace}. In \fullref{ch:Approaches} we will give a detailed explanation of the different tools we built and used to monitor and evaluate our different agents. These tools will be put into context in \fullref{ch:OurWorkflow}, where the different parts of the framework will be put to use. Finally, in \fullref{ch:AnalyzingGraphs}, we will train a Reinforcement-Learning agent using our simulation framework and take a lookt at how the different monitoring tools can help in evaluating its performance.
...
The \emph{re-buy price} is defined as the price a vendor is willing to pay a customer to buy back a used product, while the \emph{used price} is defined as the price the vendor sets for products they previously bought back and now want to sell alongside new products (whose price is defined by the \emph{new price}). In \fullref{sec:MarketScenarios}, we will explain the different market scenarios we modelled in more detail, together with how we transferred different aspects of these scenarios from the real market to our simulation framework. Please also refer to \Cref{fig:OverviewDiagram} for an overview of the different market components and how they interact in our framework.

In the context of e-commerce, Circular Economy markets are also referred to as \emph{recommerce} markets, a phrase coined in 2005~\cite{RecommerceDefinition}. From now on, when mentioning a general \emph{market} or \emph{marketplace}, we are referencing a Circular Economy marketplace with re-buy prices.
