...
\begin{figure}[ht]
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\subcaption{Mean profits achieved during the training run}\label{fig:PPOOligopolyLineProfitsAll}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[width = \textwidth]{images/experiments/PPOOligopoly/PPOOligopolyCumulativeRewards.pdf}\\
		\subcaption{Cumulative rewards per training stage, recorded during the Agent-monitoring session}\label{fig:PPOOligopolyCumulativeRewards}
	\end{subfigure}
	\caption{Training progress of the PPO-Oligopoly experiment. The agent was trained for 5,000 episodes with all agents playing at the same time. The PPO-Agent took significantly longer than the SAC-Agent (\Cref{fig:SACDuopolyProfitsMean}) to reach its maximum possible profit.}\label{fig:PPOOligopolyDiagrams1}
\end{figure}

% price rebuy + price refurb

% purch new + purch refurb

% buy nothing + throw away