...
\begin{definition}\label{def:customerSoftmax}
	Let \(R_{in}\) and \(R_{ir}\) be the preference ratios as defined in \Cref{def:customerDecisions}. Let \(S_{r}\) be the sum of those ratios:
	\[
		S_{r} := R_{in} + R_{ir}
	\]
...
\begin{definition}\label{def:ownerDecisions}
	We keep the variables defined in \Cref{def:customerDecisions}. Additionally, let \(C_{ib}\) be the price at which vendor \(i\) is willing to buy back an owner's product. \(C_{ib}\) will again be within the range of \([0,10]\). For each vendor, we define \(C_{im}\) as the purchase option with the lowest price:
	\[
