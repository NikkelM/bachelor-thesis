...
\begin{definition}\label{def:customerDecisions}
	Let \(P_{i, new}\) be the price of the new and \(P_{i, ref}\) the price of the refurbished product set by vendor \(i\), with \(0 \leq i < n\) and \(n \in \mathbb{N}\) the number of vendors in the market. Prices will always be within the range \([0,10]\). We also define \(r_{new}(P)\) as the function determining the customers' \emph{preference ratio} regarding a new price set by any vendor:
	\[
		r_{new}(P) := \frac{10}{P} - e^{P - 8}
	\]
	and similarly, \(r_{ref}(C)\) as the preference ratio regarding the refurbished price:
	\[
...
In order to be usable as a probability distribution, the different preference ratios need to add up to \(1\). \Cref{def:customerSoftmax} explains how we use the \emph{softmax} function to normalize our values, after which each preference ratio will be within the interval \((0,1)\), with all ratios adding up to \(1\) as needed. We use \emph{softmax} to normalize our values, as it is able to deal with the possibly negative results of the two \(r\)-functions. % \todo{Possibly a citation for softmax usage in RL?}
...
Following this conversion of the preference ratios, the market uses a standard \emph{multinomial} distribution to draw \(m\) samples out of the pool of probabilities \(\Pi\), with \(m \in \mathbb{N}\) being the number of customers making a purchasing decision in this step of the simulation. The result of this sampling decides how many customers buy which or no product.

The following shows the calculation of such probability distributions in an exemplary market scenario. We assume a Circular Economy, Duopoly market scenario with the prices as detailed in \Cref{tab:CustomerExamplePrices}. This will result in the following preference
\begin{wraptable}{l}{5.1cm}
	\begin{tabular}{|r|l|l|}
		\hline
		         & new & refurbished \\\hline
		Vendor 0 & 7   & 4           \\\hline
		Vendor 1 & 6   & 2           \\\hline
	\end{tabular}
	\caption{Prices set by the vendors in our example.}\label{tab:CustomerExamplePrices}
\end{wraptable}
\noindent ratios by customers...
